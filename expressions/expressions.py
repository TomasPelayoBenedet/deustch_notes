...
def count_expressions(text):
    """
    Counts the number of expressions in a text
    
    Args:
        text (str): Text to count expressions from
...
def split_data_by_word_limit(data, word_limit=70):
    """
    Splits data into chunks where each chunk has at most word_limit expressions
    
    Args:
        data (list): List of rows from CSV
        word_limit (int): Maximum number of expressions per chunk
    ...
    for row in data:
        # Count expressions in current row
        row_word_count = sum(count_expressions(cell) for cell in row)
...
def create_latex_document(csv_file_path, output_tex_file, expressions_per_table=70):
    """
    ...
    Args:
        ...
        expressions_per_table (int): Maximum number of expressions per table (default: 70)
    """
    # Read CSV
    data = []
    ...
    chunks = split_data_by_word_limit(data, expressions_per_table)
...
def main():
    ...
    csv_file = "expressions/expressions.csv"  # Change this to your CSV file name
    tex_dir = "tex"  # Directory for tex files
    tex_file = os.path.join(tex_dir, "expressions.tex")
    
    # Create LaTeX document
    create_latex_document(csv_file, tex_file, expressions_per_table=90)
